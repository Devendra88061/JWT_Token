...
class userController {
  ...
  // Delete user by ID
  public static async deleteUserById(request: Request, response: Response, next: NextFunction) {
    try {
      const userId = request.params.id;
      userService.deleteUserById(userId, (err: any, result: any) => {
        if (err) {
          next(new HttpException(400, err));
        } else {
          response.status(200).send(new HttpResponse(null, result, "Delete User", null, 1, null));
        }
      });
    }
    catch (err) {
